...
for idx_EbNo=1:length(EbNo)                          % EbNo의 길이인 20까지 1씩 증가
    ...
    %% Setting Channel
    Channel=(randn(Code_length/4,1)+(randn(Code_length/4,1))*1i)/sqrt(2);  % channel 세팅
...
for idx=1:length(EbNo_Theoretical)
    BER_Theoretical(idx)=(3.2/8)*(1-(1/sqrt(1+5/(2*EbNo_Theoretical(idx))))); % 이론적인 EbNo 계산
end
%% Figure
figure;
...
title('16QAM of Rayleigh Fading');
axis([0 50 10^-5 10^-0])
grid on

save('Con_16QAM_Fading_KBS','BER')